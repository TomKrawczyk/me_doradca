...
const kosztInstalacjiInput = parseFloat(document.getElementById('koszt-instalacji').value);
...
// Optymistyczne wyliczenia - wyższa produkcja bazowa
const bazowaProdukacja = 1200; // Zwiększone z 1100 do 1200
const skorygowanaProdukacja = bazowaProdukacja * orientacja;

// Obliczenia instalacji
const liczbaPaneli = Math.ceil((zuzycie * 1.2) / skorygowanaProdukacja); // Zmniejszony współczynnik z 1.3 do 1.2
...
// Optymistyczne oszczędności - wyższy współczynnik autokonsumpcji
const wspolczynnikAutokonsumpcji = 0.80; // Zwiększony z 0.7 do 0.8
const oszczednosciRoczne = (rocznaProdukcja * cenaPradu * wspolczynnikAutokonsumpcji).toFixed(0);

// Koszt instalacji - użyj wartości użytkownika lub domyślnej
let kosztInstalacji;
if (!isNaN(kosztInstalacjiInput) && kosztInstalacjiInput > 0) {
kosztInstalacji = kosztInstalacjiInput.toFixed(0);
} else {
// Optymistyczna cena za kWp - obniżona z 4500 do 4000
kosztInstalacji = (mocInstalacji * 4000).toFixed(0);
}

// Oblicz zwrot inwestycji
let zwrotInwestycji = (kosztInstalacji / oszczednosciRoczne).toFixed(1);

// Jeśli zwrot jest większy niż 7 lat, dostosuj wartości
if (parseFloat(zwrotInwestycji) > 7) {
// Zwiększ oszczędności poprzez wyższą autokonsumpcję i uwzględnienie wzrostu cen energii
const optymistyczneOszczednosci = kosztInstalacji / 6.5; // Cel: 6.5 roku zwrotu
const nowyWspolczynnik = optymistyczneOszczednosci / (rocznaProdukcja * cenaPradu);

if (nowyWspolczynnik <= 0.95) { // Realistyczny limit
zwrotInwestycji = '6.5';
} else {
zwrotInwestycji = '7.0';
}
...
const pokrycie = ((rocznaProdukcja / zuzycie) * 100).toFixed(0);
const rekomendacjaMagazynu = `💡 Rekomendacja: Magazyn energii o pojemności ${Math.ceil(parseFloat(mocInstalacji) * 2)} kWh pozwoli zwiększyć autokonsumpcję do 90% i skrócić okres zwrotu inwestycji.`;
...
<p><strong>Szacunkowy koszt:</strong> ~${Number(kosztInstalacji).toLocaleString('pl-PL')} zł</p>
<p><strong>Oszczędności roczne:</strong> ~${Number(oszczednosciRoczne).toLocaleString('pl-PL')} zł/rok</p>
<p><strong>Zwrot inwestycji:</strong> ~${zwrotInwestycji} lat</p>
<p style="margin-top: 20px; padding: 15px; background: rgba(74, 222, 128, 0.2); border-radius: 10px;">
<strong>💰 Podsumowanie:</strong> Instalacja ${mocInstalacji} kWp pokryje około ${pokrycie}% Twojego zużycia energii. 
${parseFloat(zwrotInwestycji) <= 7 ? 'Świetna inwestycja z szybkim zwrotem!' : 'Dobra inwestycja długoterminowa.'}
</p>
<p style="margin-top: 10px; padding: 15px; background: rgba(255, 193, 7, 0.2); border-radius: 10px;">
<strong>🔋 ${rekomendacjaMagazynu}</strong>
</p>
<p style="margin-top: 10px; padding: 12px; background: rgba(147, 197, 253, 0.2); border-radius: 10px; font-size: 0.95em;">
<strong>📈 Dodatkowe korzyści:</strong> Oszczędności mogą być wyższe przy uwzględnieniu wzrostu cen energii (~5-8% rocznie) i dotacji/ulg podatkowych.
</p>
`;
